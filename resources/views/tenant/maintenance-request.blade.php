            ...
            <div class="col-12 col-lg-5">
                <div class="card shadow-sm rounded-lg border-0">
                    ...
                    <div class="card-body">
                        ...
                        <div class="mb-3"><label class="form-label">Subject</label><input type="text"
                                class="form-control" id="subject"></div>
                        <div class="mb-3">
                    ...
                    <div class="card-body p-0">
                        <table class="table mb-0 align-middle">
                            ...
                            <tbody id="requests-body">
                                <tr>
                                    <td colspan="3" class="text-muted">No requests yet</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
...
@push('scripts')
    <script>
        ...
        function renderRequests() {
            ...
            if (!submitted.length) {
                bodyEl.innerHTML = '<tr><td colspan="3" class="text-muted">No requests yet</td></tr>';
                return;
            }
            bodyEl.innerHTML = submitted.map((r) => {
                let action = '<span class="text-muted small">—</span>';
                if (r.status === 'awaiting_confirm') action =
                    '<button class="btn btn-sm btn-success" data-confirm="' + r.id + '">Confirm</button>';
        ...
        document.getElementById('requests-body').addEventListener('click', async (ev) => {
            ...
            if (!confirmId) return;
            try {
                await apiFetch('/api/tenant/maintenance/' + confirmId + '/confirm', {
                    method: 'POST'
                });
                ...
                document.getElementById('submit-result').innerHTML = '<div class="alert alert-danger">' + e
                    .message + '</div>';
            }
        });
