            ...
            <div class="modal-content border-0 shadow-lg">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-2" id="payRefWrap">
                        ...
                        <input id="payRef" type="text" class="form-control" placeholder="e.g. QGH7XYZ12"
                            autocomplete="off">
                ...
                <div class="modal-footer border-top">
                    ...
                    <button type="button" id="payConfirm" class="btn btn-primary d-inline-flex align-items-center gap-2"
                        disabled>
                        <span id="payConfirmSpinner" class="spinner-border spinner-border-sm d-none"
                            role="status"></span>
            ...
            <div class="modal-content border-0 shadow-lg">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="tx-code" placeholder="e.g. QGH7XYZ12"
                            autocomplete="off">
                    </div>
                    <div class="mb-2">
    ...
    <script>
        (function() {
            ...
            function formatDate(v) {
                if (!v) return '—';
                const d = new Date(v);
                return isNaN(d.getTime()) ? v : d.toLocaleDateString(undefined, {
                    year: 'numeric',
                    month: 'short',
                    day: 'numeric'
                });
            ...
            function statusBadge(status, transactionId) {
                const map = {
                    ...
                    pending: {
                        label: transactionId ? 'Awaiting Verification' : 'Pending',
      ...
      <tr>
        ...
        <span class="spinner-border spinner-border-sm me-2"></span>Loading…</td></tr>`;
                ...
                try {
                    const {
                        body
                    } = await apiFetch(`/api/tenant/payments?page=${page}`);
        ...
        Submitting again will overwrite it.
            ...
            payRentBtn.addEventListener('click', async () => {
                ...
                try {
                    const {
                        body
                    } = await apiFetch('/api/tenant/pay-rent');
                    ...
                    if (!payableMonths.length) {
                        payHint.textContent = body.message ||
                            'Your rent is fully paid up to the end of your lease. 🎉';
                        return;
                    }
                ...
                try {
                    ...
                    if (month.payment_id) {
                        ...
                        ({
                            status,
                            body
                        } = await apiFetch(`/api/tenant/payments/${month.payment_id}/submit`, {
                            method: 'POST',
                            body: JSON.stringify({
                                transaction_id: ref,
                        ...
                        ({
                            status,
                            body
                        } = await apiFetch('/api/tenant/payments/init-and-submit', {
                            method: 'POST',
                            body: JSON.stringify({
                    ...
                    if (status === 202) {
                        payMsgEl.textContent = body?.message ||
            ...
            txSubmitBtn.addEventListener('click', async () => {
                ...
                try {
                    const {
                        status,
                        body
                    } = await apiFetch(`/api/tenant/payments/${id}/transaction-code`, {
                        method: 'POST',
                        body: JSON.stringify({
                            transaction_id: ref,
                            payment_method: method
                    ...
                    if (status === 202) {
                        txMsgEl.textContent = body?.message ||
                            'Saved offline. Will sync when you reconnect.';
                    ...
                    txErrEl.innerHTML =
                        `<div class="alert alert-danger py-2">${e.message || 'Submission failed.'}</div>`;
                } finally {
                    setTxLoading(false);
                }
            });
