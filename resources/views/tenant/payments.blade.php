...
<script>
    ...
    // Formats an ISO datetime string (e.g. "2026-08-05T00:00:00.000000Z")
    // into a readable date (e.g. "Aug 5, 2026"). Falls back to "-" if empty.
    ...
    async function load() {
        try {
            const page = await apiFetch('/api/tenant/payments');
            ...
            body.innerHTML = items.map((p) => {
                ...
                } else if (p.status === 'pending' && p.transaction_id) {
                    action = '<span class="text-muted small">Awaiting verification</span>';
                }
                return '<tr><td>' + formatDate(p.due_date) + '</td><td>' +
                    Number(p.amount).toLocaleString() + '</td><td>' + badge(p.status) + '</td><td>' +
                    (p.receipt_url || '-') + '</td><td class="text-end">' + action + '</td></tr>';
            }).join('');
        } catch (e) {
            document.getElementById('pay-error').innerHTML = '<div class="alert alert-danger">' + e.message + '</div>';
        }
    }
    ...
    document.getElementById('tx-submit').addEventListener('click', async () => {
        ...
        try {
            ...
            document.getElementById('tx-error').innerHTML = '<div class="alert alert-danger">' + e.message + '</div>';
        }
    });
