...
@section('content')
<div class="p-4">
    <h1 class="text-2xl font-bold text-primary mb-4">Activity & Sessions</h1>

    <ul class="nav nav-tabs mb-3" id="logTabs">
        <li class="nav-item"><button class="nav-link active" data-tab="sessions">Login Sessions</button></li>
        <li class="nav-item"><button class="nav-link" data-tab="audit">Audit Trail</button></li>
    ...
    <div class="card shadow-sm rounded-lg border-0 mb-3">
        <div class="card-body">
            <div class="row g-2" id="sessions-filters">
                <div class="col-auto">
                    <select class="form-select form-select-sm" id="f-device">
                ...
                <div class="col-auto">
                    <select class="form-select form-select-sm" id="f-active">
                        <option value="">All</option>
                        <option value="1">Active Sessions</option>
                        <option value="0">Ended Sessions</option>
                    </select>
                </div>
    ...
    <div class="card shadow-sm rounded-lg border-0">
        <div class="card-body p-0">
            <table class="table mb-0 align-middle">
                ...
                <tbody id="table-body"><tr><td class="text-muted p-3">Loading...</td></tr></tbody>
            </table>
        </div>
    </div>
</div>
...
<script>
    ...
    async function loadSessions() {
        ...
        const page = await apiFetch('/api/admin/sessions?' + params.toString());
    ...
    async function load() {
        try {
            ...
            else if (currentTab === 'audit') await loadAudit();
    ...
    document.getElementById('logTabs').addEventListener('click', (ev) => {
        ...
        document.getElementById('sessions-filters').style.display = currentTab === 'sessions' ? 'flex' : 'none';
        load();
    });
