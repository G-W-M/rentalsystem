...
@section('auth-content')
    ...
    <script>
        ...
        // Pre-fill email from the URL query string, same as the token —
        // the emailed reset link includes both, no reason to make the user retype it.
        (function prefillEmail() {
            const params = new URLSearchParams(window.location.search);
            const email = params.get('email');
            if (email) {
                document.getElementById('rp-email').value = decodeURIComponent(email);
            }
        })();
        ...
        document.getElementById('rp-btn').addEventListener('click', async () => {
            ...
            out.innerHTML = '';
            try {
                // Same fix as forgot-password: fetch CSRF cookie before posting.
                ...
                const res = await fetch('/api/reset-password', {
                    ...
                    body: JSON.stringify({
                        ...
                        password_confirmation: document.getElementById('rp-password-confirm')
                            .value,
                    }),
                });
                const body = await res.json();
                if (!res.ok) {
                    out.innerHTML = '<div class="alert alert-danger">' + (body.message || 'Reset failed.') +
                        '</div>';
                    return;
                }
